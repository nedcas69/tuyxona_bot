    ...
    try:
        ...
        if not order:
            ...
            for i in order:
                ...
                txtx += f'| User_id : {orders.user_id} | Куни: {orders.dates} | Заказ № <code>{i}</code> | ФИО: {orders.fio} | Тел: <code>{orders.tel}</code> | Тўлов тури: {orders.paytype} | Тўлов суммаси: {orders.summa} | Заказ тури:'
                if orders.noxor:
                    txtx += " Наҳор "
                if orders.kunduz:
                    txtx += ' Кундузги базм '
...
async def order_adm(message: Message, state: FSMContext):
    ...
    try:
        ...
        orders = await commands.select_order(order_id_adm)
        txt_or = ''
        if orders.pay_status:
            pay = " Тўлов қилинган ✅ "
        else:
            pay = " Тўлов қилинмаган ❌"
        txt_or += f'| Куни: {orders.dates} | Буюртма № <code>{order_id_adm}</code> | ФИО: {orders.fio} | Тел: <code>{orders.tel}</code> | Тўлов холати {pay}| Тўлов тури: {orders.paytype} | Тўлов суммаси: {orders.summa} | Буюртма тури:'
        if orders.noxor:
            txt_or += " Наҳор, "
        if orders.kunduz:
